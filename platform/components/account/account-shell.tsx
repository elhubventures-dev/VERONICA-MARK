...
import { accountNavSections } from "@/lib/account/nav";
    ...
    <div className="flex min-h-svh flex-col bg-[var(--color-background)] text-[var(--color-foreground)]">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--color-border)] bg-[var(--color-background)]/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <BrandMark href="/account" variant="icon" size={28} />
          <span className="hidden text-sm text-[var(--color-muted-foreground)] sm:inline">Account</span>
      ...
      <div className="mx-auto flex w-full max-w-[1440px] flex-1">
        ...
        <main id="account-main" className="min-w-0 flex-1 px-4 py-8 sm:px-6 lg:px-10 lg:py-10" tabIndex={-1}>
          {children}
        </main>
      </div>
      ...
      {mobileOpen ? (
        ...
        >
          <div className="absolute inset-y-0 left-0 flex w-[min(100%,20rem)] flex-col bg-[var(--color-surface)] shadow-xl">
            ...
            <nav className="flex-1 overflow-y-auto p-3" aria-label="Account">
            ...
            <div className="border-t border-[var(--color-border)] p-4">
              <Link href="/shop" className="block text-sm text-[var(--color-primary)]">
                Continue shopping
              </Link>
            </div>
          </div>
